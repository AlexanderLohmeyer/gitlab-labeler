...
describe("getConfig", () => {
    beforeEach(() => {
        ...
        jest.mock(path_1.default.resolve("./labeler-config.js"), () => ({
            directoriesLabels: [
                [/([1]).*/g, "Dir 1"],
    ...
    test("returns the merged configuration", () => {
        ...
        const result = (0, get_config_1.getConfig)();
    ...
    test("throws an error when GITLAB_ACCESS_TOKEN is not set", () => {
        ...
        process.env.CI_SERVER_HOST = "my_server_host";
        expect(() => (0, get_config_1.getConfig)()).toThrow("Gitlab access token not set. Set GITLAB_ACCESS_TOKEN environment variable.");
    });
    test("throws an error when CI_SERVER_HOST is not set", () => {
        process.env.GITLAB_ACCESS_TOKEN = "my_access_token";
        delete process.env.CI_SERVER_HOST;
        expect(() => (0, get_config_1.getConfig)()).toThrow("Gitlab CI Environment variables not set. You propably tried to run this tool outside of Gitlab CI");
    });
});
